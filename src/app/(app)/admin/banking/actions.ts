...
// Phase 2 note: creating an account will additionally auto-create its
// Chart-of-Accounts ledger account and post the opening balance entry;
// archiving will require the account to be fully reconciled first.
...
export async function createBankAccount(formData: FormData) {
  ...
  await auditedTransaction(async (tx) => {
    ...
    await audit(tx, {
      ...
      targetId: account.id,
      summary: `Created bank account ${data.nickname} under ${entity.code}`,
      after: {
        entity: entity.code,
...
export async function createCashLocation(formData: FormData) {
  ...
  await auditedTransaction(async (tx) => {
    ...
    await audit(tx, {
      ...
      summary: `Created cash location "${data.name}" under ${entity.code}`,
      after: { entity: entity.code, name: data.name },
    })
  })
  revalidatePath('/admin/banking')
}
