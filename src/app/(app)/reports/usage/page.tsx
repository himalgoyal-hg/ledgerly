  ...
  return (
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-3">
        {([
          ['Paid through banks', tb, pct(tb, tc) + ' of spend'],
          ...(showCash ? [['Paid in cash', tc, pct(tc, tb) + ' of spend']] : []),
          ['Total outflow', tb + tc, 'last 12 months'],
        ] as [string, number, string][]).map(([l, v, h]) => (
        ...
        <table className="w-full text-sm">
          <thead className={theadClass}>
            <tr>
              ...
              <th className="px-3 py-2 text-right">Through bank</th>
              {showCash && <th className="px-3 py-2 text-right">In cash</th>}
              {showCash && <th className="px-3 py-2 text-right">Cash share</th>}
            </tr>
          </thead>
          <tbody className="divide-y divide-line-2">
            {rows.map((r, i) => (
              <tr key={i} className="hover:bg-surface-2/60">
                ...
                {showCash && <td className="px-3 py-2 text-right tabular-nums">{inr(r.cash)}</td>}
                {showCash && <td className="px-3 py-2 text-right text-ink-2">{pct(r.cash, r.bank)}</td>}
              </tr>
            ))}
